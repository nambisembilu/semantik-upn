        ...
        <div class="page-header-content d-lg-flex">
            <div class="d-flex">
                <h4 class="page-title mb-0">
                    Kinerja - <span class="fw-normal">JPT</span>
                </h4>
                                ...
                                @if(!empty($mainSkp) && count($mainSkp) > 0)
                                    ...
                                    @endif
                                        ...
                                        <td class="text-left" style="width:20%;">
                                                ...
                                                onclick="AddUpdateTaskFromModal({{$skpWorkPlan->id}}, '{{$skpWorkPlan->title}}', 'Sekretaris Jenderal', '{{$skpWorkPlan->get_task_from}}')"
                                                >
                                                    <i class="ph-star"></i>
                                                </button>
                                                <button type="button" class="btn btn-sm btn-success" title="Edit Hasil Kerja Utama" 
                                                onclick="OpenMainWorkPlanModal({{$skpWorkPlan->id}},'{{!empty($skpWorkPlan->employment_agreement_id)}}')">
...
function RefreshInterventionAssignment() {
    $.ajax({
        url: "{{route('modules.performance.skp-plan.get_employment_agreement')}}",
            method: 'POST',
            data: 
            {
                _token: $("input[name='_token']").val(),
        ...
        url: "{{route('modules.performance.skp-plan.get_work_plan_by_id')}}",
            ...
            success: function (response) {
                ...
                else
                {
                    ...
                    $('#intervention_indicator').val(response.employment_agreement_id);
                    $('#cascading_work_plan_indicator_container').empty();
...
function AddUpdateTaskFromModal(id, skpPlan, defaultTaskFrom, getTaskFrom) {
...
$(document).ready(function() {
    ...
    $("#formAddCascadingWorkPlan").submit(function(e){
        ...
        //execute submit
        $.ajax({
            url: "{{route('modules.performance.skp-plan.create_cascading_skp_work_indicator_plan_for_jpt')}}",
            type:'POST',
            contentType: false,
            processData: false,
            data: formData,
