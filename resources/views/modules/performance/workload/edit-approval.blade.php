...
@section('toolbar-action')
    ...
    <div class="d-flex">
        <!--begin::Menu-->
        <div class="me-0">
            @if($work_position->level>1)
                <a href="{{route('modules.performance.workload.detail-approval',$personal_workload->id)}}" class="btn btn-bg-light btn-sm btn-icon-primary btn-text-primary my-1">
                                ...
                                <div class="card-title flex-column justify-content-start">
                                    ...
                                    <select name="lead_id" data-control="select2" data-placeholder="Pilih Atasan..." class="form-select form-select-solid form-select-sm w-lg-400px w-sm-250px">
                                        ...
                                        @foreach($personal_leads as $personal_lead)
                                            <option value="{{$personal_lead->id}}" @if($personal_workload->lead_id==$personal_lead->id) selected @endif>{{$personal_lead->name}} - {{$personal_lead->unit->name}}</option>
                                        @endforeach
                                    </select>
                                    ...
                                    @if(count($workload_activities)>0)
                                        <tr>
                                            <td class="text-center" colspan="14">
                                                ...
                                                <button type="button" data-bs-toggle="modal" data-bs-target="#modal_list_activity" class="btn btn-secondary btn-sm">
                                                    <i class="bi bi-list"></i> Lihat Daftar Kegiatan
                                                </button>
